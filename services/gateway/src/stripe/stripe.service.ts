...
export class StripeService {
  ...
  async createCheckoutSession(userId, crownPackageId) {
    return await this.clientStripeService.send(
      { cmd: "create-checkout-session-stripe-service" },
      { userId: userId, crownPackageId: crownPackageId },
    )
  }

  async webhook(signature, payload) {
    return await this.clientStripeService.send(
      { cmd: "webhook-stripe-service" },
